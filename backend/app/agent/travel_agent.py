...
# Wrap as Agents SDK tools
@function_tool
def get_weather_tool(city: str, date: Optional[str] = None):
    ...
    tool_call_log.append({"tool": "weather", "data": result})
    return result
...
def search_flights_tool(origin: str, destination: str, date: str):
    ...
    tool_call_log.append({"tool": "flights", "data": result})
    return result

@function_tool
def search_hotels_tool(city: str, check_in: str, check_out: str):
    ...
    tool_call_log.append({"tool": "hotels", "data": result})
    return result

@function_tool
async def save_preference_tool(user_id: str, key: str, value: str):
    """Save a long-term travel preference for the user.
    
    Stores preferences like seat preference, budget range, preferred airlines, etc.
    """
    await save_preference(user_id, key, value)
    result = f"Preference {key} set to {value} for user {user_id}."
    tool_call_log.append({"tool": "preferences", "data": result})
    return result

@function_tool
async def get_preferences_tool(user_id: str):
    """Retrieve all long-term travel preferences for the user.
    
    Returns a dictionary of preferences such as seat preference, budget range, preferred airlines.
    """
...
@function_tool
def send_trip_email_tool(subject: str, body: str) -> str:
    ...
    """
    result = send_email(current_user_email, subject, body)
    tool_call_log.append({"tool": "email", "data": {"to": current_user_email, "subject": subject}})
    return f"Trip details emailed to {current_user_email}"

# Create the agent
travel_agent = Agent(
    name="TravelPlannerAgent",
instructions="""You are a helpful travel planning assistant. 
Use the available tools to get weather information, search for flights, and find hotels.
...
At the start of helping with a new request, check existing preferences using the authenticated user_id provided in the system context. Never use "demo_user".
...
if __name__ == "__main__":
    import asyncio
    async def main():
        result = await Runner.run(travel_agent, "What's the weather in Paris and find me a flight from New York to Paris on 2026-11-01")
