...
from app.schemas.chat import ChatRequest, ChatResponse, SessionSummary, SessionMessage, SessionUpdateRequest
...
async def chat_endpoint(
    ...
    result = await Runner.run(travel_agent, input=history)
    # Capture tool calls made during this run
    calls = list(tool_call_log)

    # Save assistant message
    assistant_msg = Message(
        session_id=session_id, role="assistant", content=result.final_output
    )
    db.add(assistant_msg)
    db.commit()

    return ChatResponse(reply=result.final_output, session_id=str(session_id), tool_calls=calls)
...
async def list_sessions(
    ...
    for sess in sessions:
        ...
        result.append(SessionSummary(id=str(sess.id), created_at=sess.created_at, preview=preview, pinned=sess.pinned))
    return result
...
async def get_session_messages(
    ...
    return [SessionMessage(role=m.role, content=m.content) for m in messages]
